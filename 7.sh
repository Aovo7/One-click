#!/bin/bash
# ------------------------
# 本脚本由 "One-click by 7" 提供。尽管我们尽力确保脚本的安全性和有效性，
# 但我们不对由于使用此脚本而可能引起的任何损害或数据丢失承担责任。
# 本脚本中部分内容来自互联网，因此对这些内容的准确性、可靠性或者适用性不作保证。
# 使用本脚本即表示您理解并同意承担所有相关风险。
# 在使用任何网络脚本之前，我们建议您先进行充分的研究和测试，遵守适用的法律法规，并尊重原作者的版权和许可协议。
# ------------------------
